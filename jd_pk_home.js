...
!(async () => {
    if (!cookiesArr[0]) {
        $.msg(
            $.name,
            "銆愭彁绀恒€戣鍏堣幏鍙栦含涓滆处鍙蜂竴cookie\n鐩存帴浣跨敤NobyDa鐨勪含涓滅鍒拌幏鍙�",
            "https://bean.m.jd.com/", {
...
async function main() {
    await getToken();
    console.log("褰撳墠token锛�" + $.token);
    if ($.token) {
        await getPin();
        if ($.pin) {
            console.log("褰撳墠pin锛坧k鐮侊級锛�" + $.pin);
        }
        await checkRisk();

        let myScore = await getScore($.pin);
        console.log("鎴戠殑浜韩鍊�:" + myScore);
        $.pinList = await getFriendPinList();
        let winCnt = 0;
        if ($.pinList.length > 0) {
            console.log("寰卲k鐨刾in list:\n" + $.pinList);
            for (let i = 0; i < $.pinList.length; i++) {
                let pin = $.pinList[i];
                console.log('鍒汉鐨勭殑pin锛�' + pin);
                let fscore = await getScore(pin);
                console.log("鍒汉鐨勪含浜€�:" + fscore);
                if (fscore < myScore) {
...
function getUserFriendsPage(pageNo = 1, pageSize = 10) {
    console.log(`鑾峰彇濂藉弸鍒嗛〉鍒楄〃 pageNo: ${pageNo}, pageSize: ${pageSize}`);
    return new Promise(resolve => {
        let options = {
            ...
            try {
                if (err) {
                    ...
                    if (data) {
                        ...
                        if (bizData.state === 3 && bizData.msg === '浠婃棩娆℃暟宸茶€楀敖') {
                            $.canPk = false; // pk娆℃暟鑰楀敖
                        }
                    } else {
                        $.log('浜笢鏈嶅姟鍣ㄨ繑鍥炵┖鏁版嵁');
                    }
            ...
            try {
                ...
                if (res) {
                    ...
                    if (data.success) {
                        ...
                        console.log("鎬昏儨鍦�:" + data.data.totalWins);
                    }
                    ...
                    if (data.success) {
                        ...
                        if ($.openAwards) {
                            $.openAwards.forEach(item => {
                                console.log('鑾峰緱濂栧姳:' + JSON.stringify(item));
                            });
                        }
                    }
