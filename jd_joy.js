/**
 ...
 IOS鐢ㄦ埛鏀寔浜笢鍙岃处鍙�,NodeJs鐢ㄦ埛鏀寔N涓含涓滆处鍙�
 鏇存柊鏃堕棿锛�2021-06-21
 娲诲姩鍏ュ彛锛氫含涓淎PP鎴戠殑-瀹犳豹姹�

 瀹屾垚搴� 1%锛岃鐢ㄧ殑鎵嬪姩鎵ц锛屽厛涓嶅姞cron浜�
 榛樿80锛�10銆�20銆�40銆�80鍙€�
 export feedNum = 80
 榛樿鍙屼汉璺�
 export JD_JOY_teamLevel = 2
 */

const $ = new Env("瀹犳豹姹簩浠ｇ洰")
console.log('\n====================Hello World====================\n')
        ...
        try {
          ...
          res.on('end', () => {
            try {
              ...
              console.log('鐢熸垚楠岃瘉鐮佸繀椤讳娇鐢ㄥぇ闄咺P')
            }
          })
        } catch (e) {
        }
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (!require('./JS_USER_AGENTS').HelloWorld) {
        console.log(`\n銆愪含涓滆处鍙�${$.index}銆�${$.nickName || $.UserName}锛氳繍琛岀幆澧冩娴嬪け璐n`);
        continue
      }
      ...
      for (let tp of tasks.datas) {
        ...
        if (tp.taskName === '娴忚棰戦亾') {
          for (let i = 0; i < 3; i++) {
            console.log(`\t绗�${i + 1}娆℃祻瑙堥閬� 妫€鏌ラ仐婕廯)
            let followChannelList = await getFollowChannels();
            for (let t of followChannelList['datas']) {
              if (!t.status) {
                console.log('鈹�', t['channelName'])
        ...
        if (tp.taskName === '鍏虫敞鍟嗗搧') {
          for (let t of tp.followGoodList) {
            if (!t.status) {
              console.log('鈹�', t.skuName)
              await doTask(`sku=${t.sku}`, 'followGood')
              await $.wait(5000)
            }
          }
        }
        if (tp.taskName === '鍏虫敞搴楅摵') {
  ...
  return new Promise(resolve => {
    $.post({
      ...
      try {
        ...
        data = JSON.parse(data);
        data.success ? console.log('\t浠诲姟鎴愬姛') : console.log('\t浠诲姟澶辫触', JSON.stringify(data))
      } catch (e) {
        $.logErr(e);
      } finally {
        resolve();
      ...
      if (new Date().getTime() - new Date(data.data.lastFeedTime) < 10800000) {
        ...
        $.get({
          ...
          try {
            // console.log('鍠傞', data)
            data = JSON.parse(data);
            data.errorCode === 'feed_ok' ? console.log(`\t鍠傞鎴愬姛锛乣) : console.log('\t鍠傞澶辫触', JSON.stringify(data))
          } catch (e) {
            $.logErr(e);
          } finally {
            resolve();
...
function award(taskType) {
  return new Promise(resolve => {
    $.get({
      ...
      try {
        console.log('棰嗗彇濂栧姳', data)
        data = JSON.parse(data);
        data.errorCode === 'received' ? console.log(`\t浠诲姟鎴愬姛锛佽幏寰�${data.data}鐙楃伯`) : console.log('\t浠诲姟澶辫触', JSON.stringify(data))
      } catch (e) {
        $.logErr(e);
      } finally {
        resolve();
...
function run(fn = 'match') {
  ...
  return new Promise(resolve => {
    $.get({
      ...
      try {
        if (fn === 'receive') {
          console.log('棰嗗彇璧涜窇濂栧姳锛�', data)
        } else {
          console.log('璧涜窇', data)
          data = JSON.parse(data);
          let race = data.data.petRaceResult
          if (race === 'participate') {
            console.log('鍖归厤鎴愬姛锛�')
          } else if (race === 'unbegin') {
            console.log('杩樻湭寮€濮嬶紒')
...
function Env(t, e) {
  ...
  return new class {
    constructor(t, e) {
      this.name = t, this.http = new s(this), this.data = null, this.dataFile = "box.dat", this.logs = [], this.isMute = !1, this.isNeedRewrite = !1, this.logSeparator = "\n", this.startTime = (new Date).getTime(), Object.assign(this, e), this.log("", `馃敂${this.name}, 寮€濮�!`)
    }
    ...
    logErr(t, e) {
      ...
      s ? this.log("", `鉂楋笍${this.name}, 閿欒!`, t.stack) : this.log("", `鉂楋笍${this.name}, 閿欒!`, t)
    }
    ...
    wait(t) {
      ...
      this.log("", `馃敂${this.name}, 缁撴潫! 馃暃 ${s} 绉抈), this.log(), (this.isSurge() || this.isQuanX() || this.isLoon()) && $done(t)
    }
  }(t, e)
}
